/**
 * Regressions: the name question must not persist nickname introducers as
 * profile.name. "I'm called Maya" / "I go by Alex" historically stored
 * Called / I because the parser strips I'm then takes the next token, and
 * there is no later UI to change the name.
 *
 * Distinct from interview consent words (yes/ok/sure, PR #38), hedges
 * ("I'm not sure", "call me later", PR #41), call-me / I'm wrappers
 * ("Just call me Alex", PR #43), and "this is <name>" (PR #46). This is
 * the called / named / I-go-by copula.
 *
 * Run from repo root:
 *   node scripts/check_interview_name_called.js
 */
...
async function main() {
  for (const [text, expect] of [
    ...
    ["It's Priya", "Priya"],
    ["I'm called Maya", "Maya"],
    ["I am called Maya", "Maya"],
    ["I'm called Alex.", "Alex"],
    ["I am called Jordan", "Jordan"],
    ["I'm named Priya", "Priya"],
    ["I am named Sam", "Sam"],
    ["I go by Maya", "Maya"],
    ["I go by Alex", "Alex"],
    ["i go by Jordan", "Jordan"]
  ]) {
  ...
  console.log("OK interview-name called/go-by checks passed");
}
