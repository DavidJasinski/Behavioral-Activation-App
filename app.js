...
// Create intents always persist via executeIntent/composeReply. Noun uses of
// "plan" and first-person future intentions are not Coach write commands —
// they must not invent plan items.
function isNonRequestCreate(s) {
  const t = String(s || "");

  // Noun "plan": "my plan is to walk", "the plan includes a walk",
  // "I have a plan to cook", "talk about my plan, not walk yet".
  if (/\b(my|the|our|a|this|that|your)\s+plan\b/i.test(t)) return true;
  if (/\bplan\s+(is|was|includes|involves|for)\b/i.test(t)) return true;

  // First-person future intention without addressing the Coach as actor.
  // "I will add a walk later", "I'm going to schedule a walk",
  // "I'll create an exposure for that".
  if (
    /\b(i|we)('ll|\s+will|\s+am\s+going\s+to|\s+are\s+going\s+to|'m\s+going\s+to)\s+(add|create|set|schedule|plan|build)\b/i.test(
      t
    ) &&
  ...
  // Conditional / habitual self-talk: "when I plan a walk I feel pressure".
  if (/\b(when|if|whenever)\s+(i|we)\s+(add|create|set|schedule|plan|build)\b/i.test(t)) {
    return true;
  }
...
const INTENTS = [
  ...
  },
  {
    name: "add_goal",
    test: (s) =>
      !isNonRequestCreate(s) && /\b(add|create|set)\b.*\bgoal\b/i.test(s)
  },
