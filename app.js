...
// Create intents always persist via executeIntent/composeReply. Educational or
// self-advice questions ("how do I create…", "should I schedule…") are not
// Coach write commands — they must not invent plan items.
function isCreateQuestion(s) {
  const t = String(s || "");
  if (
    /\b(how (do|can|should|would) (i|you|one|we)|how does one|how to)\b/i.test(t)
  ) {
    return true;
  }
  if (/\bwhat('?s| is) the (best )?way to\b/i.test(t)) return true;
  if (/\bwhat does it mean to\b/i.test(t)) return true;
  // Self-directed advice questions without addressing the Coach as actor.
  if (
    /\b(should|shouldn'?t) (i|we)\b/i.test(t) &&
    !/\b(you|coach|please)\b/i.test(t)
  ) {
    return true;
  }
  if (/\b(is it better|would it help|does it help|do i need) to\b/i.test(t)) {
    return true;
  }
  return false;
}
...
const INTENTS = [
  ...
  },
  {
    name: "add_goal",
    test: (s) =>
      !isCreateQuestion(s) && /\b(add|create|set)\b.*\bgoal\b/i.test(s)
  },
