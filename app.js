...
// "Everyone calls me Maya" / "I prefer to be called Alex" are natural
// answers to the name question. The start-anchored I'm / call me strip
// then takes the first token, so those persist Everyone / I, and there
// is no later UI to change it. Prefer these phrases anywhere in the
// reply; fall back to the original start-anchored strip.
// If merging with PR #43/#48, keep these matchers before generic
// `call me` / `i am|i'm` captures (`\bcall me\b` does not match `calls me`,
// and `I'm called` does not match `prefer to be called`).
function interviewNameToken(text) {
  const raw = String(text || "").trim();
  const callsMe = raw.match(/\bcalls me\s+([a-z'-]{2,30})\b/i);
  if (callsMe) return callsMe[1];
  const preferCalled = raw.match(/\bprefer(?:s)? to be called\s+([a-z'-]{2,30})\b/i);
  if (preferCalled) return preferCalled[1];
  const iPrefer = raw.match(/\bi prefer\s+([a-z'-]{2,30})\b/i);
  if (iPrefer && !/^(to|not|be|being|a|the|it|you|we|my)$/i.test(iPrefer[1])) {
    return iPrefer[1];
  }
