...
function loadState() {
  const primary = readStoredState(STORAGE_KEY);
  if (primary) {
  ...
  if (legacy) {
    ...
    // Promotion is best-effort. If quota or browser policy blocks the new key,
    // keep using the readable legacy key instead of discarding local-only data.
    if (writeStorage(STORAGE_KEY, legacyRaw)) {
      activeStorageKey = STORAGE_KEY;
...
function saveState() {
  ...
  if (activeStorageKey === LEGACY_KEY) {
    if (!writeStorage(LEGACY_KEY, raw)) {
      notifyStorageFailure();
      return false;
    }
    if (writeStorage(STORAGE_KEY, raw)) {
      activeStorageKey = STORAGE_KEY;
      removeStorage(LEGACY_KEY);
    }
    return true;
...
function readStorage(key) {
  try {
    ...
    console.warn(`Could not read state from ${key}`, e);
    return null;
  }
}

function writeStorage(key, raw) {
  try {
    localStorage.setItem(key, raw);
    return true;
  } catch (e) {
    console.warn(`Could not write state to ${key}`, e);
    return false;
  }
}
...
function removeStorage(key) {
  try {
    ...
    console.warn(`Could not remove old state key ${key}`, e);
  }
}

function readStoredState(key) {
  return parseStoredState(readStorage(key), key);
}
...
function parseStoredState(raw, key) {
  ...
  try {
    ...
    console.warn(`Could not load state from ${key}`, e);
    return null;
  }
}
...
function hydrateState(parsed) {
  const merged = Object.assign(structuredClone(DEFAULT_STATE), parsed, {
    ...
    coach: Object.assign(
      structuredClone(DEFAULT_STATE.coach),
      parsed.coach || {}
    )
  });
...
function notifyStorageFailure() {
  ...
  if (typeof window !== "undefined" && typeof window.alert === "function") {
    window.alert(
      "Break Free could not save changes in this browser storage area. " +
        "Please export or copy anything important before closing this tab."
    );
  }
}
