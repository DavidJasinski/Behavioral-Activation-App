...
// "My nickname is Maya" / "Nickname's Alex" are natural answers to the name
// question. The start-anchored I'm / call me / my name is strip then takes the
// first token, so those persist My / Nickname, and there is no later UI to
// change it. Prefer nickname phrases anywhere in the reply; fall back to the
// original start-anchored strip.
// Distinct from PR #43 (`\bmy name is\b` / `\bname'?s\b` — "nickname is" is
// not "name is", and `\bname'?s\b` does not match inside "nickname's"),
// PR #48 (`I'm called` / `I go by`), and PR #52 (`calls me` /
// `prefer to be called`). If merging with those, keep this matcher in
// interviewNameToken alongside the other phrase extractors.
function interviewNameToken(text) {
  const raw = String(text || "").trim();
  const nick = raw.match(
    /\b(?:my\s+)?nickname(?:'s|\s+is)\s+([a-z'-]{2,30})\b/i
  );
  if (
    nick &&
    !/^(the|a|an|not|just|really|actually|probably|only|still|my|me)$/i.test(
      nick[1]
    )
  ) {
    return nick[1];
  ...
  const cleaned = raw.replace(
    /^(i'?m|my name is|call me|it's|im|name's)\s+/i,
    ""
  );
