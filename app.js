...
function promoteLegacyState(raw) {
  try {
    localStorage.setItem(STORAGE_KEY, raw);
    localStorage.removeItem(LEGACY_KEY);
    activeStorageKey = STORAGE_KEY;
  } catch (e) {
    console.warn("Could not promote legacy state; continuing with legacy storage", e);
...
function saveState() {
  ...
  if (activeStorageKey === LEGACY_KEY) {
    promoteLegacyState(raw);
    if (activeStorageKey === STORAGE_KEY) return;
  }
