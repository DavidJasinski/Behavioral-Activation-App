...
function loadState() {
  ...
  if (!current && !legacy) return structuredClone(DEFAULT_STATE);
  if (!legacy) return current;

  const currentScore = current ? stateRichness(current) : -1;
  const legacyScore = stateRichness(legacy);
  const preferred = legacyScore > currentScore ? legacy : current;
  const fallback = preferred === legacy ? current : legacy;
  const merged = fallback ? mergeStoredStates(preferred, fallback) : preferred;

  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(merged));
    localStorage.removeItem(LEGACY_KEY);
  } catch (e) {
    console.warn("Could not promote legacy state", e);
  }

  return merged;
}

function parseStoredState(raw, label) {
  if (!raw) return null;
  try {
    return normalizeState(JSON.parse(raw));
  } catch (e) {
    console.warn(`Could not load ${label} state`, e);
    return null;
  }
}
...
function stateRichness(state) {
  ...
  return (
    ...
    state.activations.length * 4 +
    state.logs.length * 2 +
    (c.memory || []).length +
    (c.facts || []).length +
    Object.keys(c.topicCounts || {}).length +
    [
      p.name,
    ...
    [
      ...
      "bestTimes",
      "interviewNotes"
    ].reduce((sum, key) => sum + (Array.isArray(p[key]) ? p[key].length : 0), 0)
  );
}

function mergeStoredStates(preferred, fallback) {
  const merged = structuredClone(preferred);
  merged.goals = mergeRecords(preferred.goals, fallback.goals);
  merged.activations = mergeRecords(preferred.activations, fallback.activations);
  merged.logs = mergeRecords(preferred.logs, fallback.logs);

  merged.profile = mergeProfile(preferred.profile, fallback.profile);
  merged.user = Object.assign({}, fallback.user, preferred.user);
  merged.preferences = Object.assign({}, fallback.preferences, preferred.preferences);
  merged.coach = mergeCoach(preferred.coach, fallback.coach);
...
function mergeRecords(primary = [], secondary = []) {
  ...
  [...primary, ...secondary].forEach((item) => {
    const key = item && (item.id || `${item.ts || item.createdAt || ""}:${item.type || ""}:${item.content || item.title || ""}`);
...
function mergeProfile(primary = {}, secondary = {}) {
  ...
  [
    ...
    merged[key] = mergeScalars(primary[key], secondary[key]);
  });
  merged.interviewProgress = Object.assign(
    {},
  ...
  merged.interviewProgress.openCloseAsked =
    !!(
      (primary.interviewProgress && primary.interviewProgress.openCloseAsked) ||
...
function mergeCoach(primary = {}, secondary = {}) {
  return Object.assign({}, secondary, primary, {
    memory: mergeRecords(primary.memory, secondary.memory),
    facts: mergeRecords(primary.facts, secondary.facts),
    topicCounts: Object.assign({}, secondary.topicCounts, primary.topicCounts)
  });
}

function mergeScalars(primary = [], secondary = []) {
