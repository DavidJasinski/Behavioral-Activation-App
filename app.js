...
function loadState() {
  try {
    ...
    if (legacy) {
      if (!current) {
        selected = legacy;
      } else if (legacy.raw !== current.raw) {
        const legacyScore = storedStateScore(legacy.parsed);
        const currentScore = storedStateScore(current.parsed);
        selected = legacyScore > currentScore ? legacy : current;
        carryForward = selected === legacy ? current : legacy;
      }
    }
    if (!selected) return structuredClone(DEFAULT_STATE);

    const merged = mergeStoredState(selected.parsed);
    if (carryForward) mergeStateData(merged, mergeStoredState(carryForward.parsed));
    activeStorageKey = selected.key;

    if (selected.key === LEGACY_KEY || carryForward?.key === LEGACY_KEY) {
      try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify(merged));
        activeStorageKey = STORAGE_KEY;
        try { localStorage.removeItem(LEGACY_KEY); } catch {}
      } catch (e) {
        console.warn("Could not promote legacy state; continuing with legacy fallback", e);
...
function readStoredState(key) {
  ...
  if (!raw) return null;
  try {
    return { key, raw, parsed: JSON.parse(raw) };
  } catch (e) {
    console.warn(`Could not parse state from ${key}`, e);
    return null;
  }
}
...
function storedStateScore(parsed) {
  ...
  const collectionScore = arrays.reduce(
    (sum, value) => sum + (Array.isArray(value) ? value.length : 0),
    0
  );
  const topicScore = c.topicCounts ? Object.keys(c.topicCounts).length : 0;
  const profileScore = [
    p.name,
    ...
    p.interviewComplete,
    p.interviewSkipped,
    p.interviewProgress?.currentTopic,
    p.interviewProgress?.openCloseAsked
  ].filter(Boolean).length;
  return collectionScore + topicScore + profileScore;
}

function mergeStateData(base, other) {
  base.goals = mergeUniqueItems(base.goals, other.goals);
  base.activations = mergeUniqueItems(base.activations, other.activations);
  base.logs = mergeUniqueItems(base.logs, other.logs);
  base.coach.memory = mergeUniqueItems(base.coach.memory, other.coach.memory, memoryKey);
  base.coach.facts = mergeUniqueItems(base.coach.facts, other.coach.facts, factKey);
  base.coach.topicCounts = mergeTopicCounts(base.coach.topicCounts, other.coach.topicCounts);
  base.preferences = Object.assign({}, other.preferences || {}, base.preferences);
  base.user = Object.assign({}, other.user || {}, base.user);

  mergeProfileData(base.profile, other.profile || {});
  return base;
}

function mergeProfileData(base, other) {
  [
    "struggles",
    "struggleNotes",
    "avoiding",
    ...
    base[field] = mergeUniqueItems(base[field], other[field]);
  });
  ["name", "communicationStyle", "challengeLevel"].forEach((field) => {
    if (!base[field] && other[field]) base[field] = other[field];
  });
  base.interviewProgress = base.interviewProgress || { askedTopics: [], currentTopic: null, openCloseAsked: false };
}

function mergeUniqueItems(primary = [], secondary = [], keyFn = defaultMergeKey) {
  const out = [];
  const seen = new Set();
...
function memoryKey(item) {
  return item && typeof item === "object"
    ? `${item.ts || ""}|${item.role || ""}|${item.text || ""}`
    : defaultMergeKey(item);
}
...
function mergeTopicCounts(primary = {}, secondary = {}) {
  const merged = Object.assign({}, primary || {});
  Object.entries(secondary || {}).forEach(([topic, count]) => {
    merged[topic] = Math.max(Number(merged[topic]) || 0, Number(count) || 0);
  });
  return merged;
}
...
function saveState() {
  ...
  try {
    ...
    if (activeStorageKey === LEGACY_KEY) {
      try { localStorage.removeItem(LEGACY_KEY); } catch {}
    }
    activeStorageKey = STORAGE_KEY;
  } catch (e) {
    if (activeStorageKey !== LEGACY_KEY) throw e;
